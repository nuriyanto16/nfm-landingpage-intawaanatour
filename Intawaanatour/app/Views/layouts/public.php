...
<body>
  <?php $promoText = t(setting('promo_text_id', ''), setting('promo_text_en', '')); ?>
  <?php if (setting('promo_active') === '1' && trim($promoText) !== ''): ?>
    <a class="promo-bar" href="<?= base_url(setting('promo_url', 'trips') ?: 'trips') ?>">
      <span class="promo-bar__tag">★</span>
      <span class="promo-bar__text"><?= esc($promoText) ?></span>
      <span class="promo-bar__cta"><?= t('Lihat Trip', 'View Trips') ?> →</span>
    </a>
  <?php endif ?>
  ...
  <a class="wa-float" href="<?= wa_link(t('Halo Inta Waana Tour, saya ingin bertanya tentang trip Labuan Bajo.', 'Hi Inta Waana Tour, I would like to ask about Labuan Bajo trips.')) ?>" target="_blank" rel="noopener" aria-label="WhatsApp">
