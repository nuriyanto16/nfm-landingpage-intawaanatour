...
- [x] **HARDENING KEAMANAN + CAPTCHA login** — (1) **CSRF** diaktifkan global (`Filters.php` before: `csrf`,`invalidchars`), mode **session** + `tokenRandomize=true` (`Security.php`); semua form sudah punya `csrf_field()`. (2) **Login**: CAPTCHA gambar GD mandiri (`Controllers/Captcha.php`, route `admin/captcha`, jawaban di sesi, sekali pakai, kedaluwarsa 5 mnt) + **throttle** 5/menit/IP (CI4 throttler) + **session regenerate** anti-fixation + validasi input (`Admin/Auth.php`); UI captcha + tombol reload di `login.php`. (3) **Header keamanan** di `public/.htaccess`: CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, unset X-Powered-By, blok dotfile/.env/.sql/.md dll. (4) **Cookie** `secure` otomatis aktif di produksi (`Cookie.php`). (5) **Upload** `save_upload()` whitelist MIME+ekstensi & batas 5MB. SQLi sudah aman (Query Builder berparameter, `allowedFields`, tanpa raw SQL). Diuji E2E lokal **dan live**: login benar→Dashboard 200; captcha salah/ CSRF kosong→ditolak (booking & login no-CSRF → 403); semua halaman publik 200; header tampil di URL publik. ⚠️ Catatan: container app masih `CI_ENVIRONMENT=development` — disarankan `production` untuk situs publik (sembunyikan error + aktifkan secure cookie).
